...
class Host:
    ...
    def udt_send(self, dst_addr, data_S):
        # if the message is more than 19 char
        if len(data_S) > (19):
            ...
            while len_remain > 0:
                ...
                data_split = str(id) + str(fragflag) + str(offset) + data_split
                p = NetworkPacket(dst_addr, data_split)
                self.out_intf_L[0].put(p.to_byte_S())  # send packets always enqueued successfully
                print('%s: sending packet "%s" on the out interface with mtu=%d' % (self, p, self.out_intf_L[0].mtu))
            offset = 0
            return
        # packet doesn't need to be split
        else:
    ...
    def udt_receive(self):
        ...
        if pkt_S is not None:
            ...
            offset = pkt_S[7:9]
            offset = int(offset)
            msgTuple = (destination, id, fragflag, offset, pkt_S[9:])
            self.msgBuffer.append(msgTuple)
            print('%s: received packet "%s" on the in interface' % (self, pkt_S))
            if fragflag == 0:
                sorted(self.msgBuffer, key=lambda packet: packet[3])
                something = ""
